...
class Project < ActiveRecord::Base
  ...
  def self.add(options)
    ...
    if project.checkout
      project.save!
      project.create_local_conf(options[:local_environment_variables])
      project
    end
  end
  ...
  def create_local_conf(local_environment_variables)
    local_config_path = path('goldberg_config.rb')
    local_config_file = File.open(local_config_path, 'w+')
    local_config_file.puts <<-EOS
      Project.configure do |config|
        config.environment_variables = #{local_environment_variables}
      end
    EOS
  end

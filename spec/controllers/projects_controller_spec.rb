...
describe ProjectsController do
  ...
  it "loads one project" do
    ...
    Factory(:builder, :user => controller.current_user, :project => project)
    get :show, :project_name => project.name
    response.should be_ok
    assigns[:project].should == project
  end

  it "allows forcing a build only if user has building rights" do
    project = Factory.create(:project, :name => 'some_project')
    Factory(:builder, :user => controller.current_user, :project => project)
  ...
  it "does not allow forcing a build if user has no building rights" do
    project = Factory.create(:project, :name => 'some_project')
    Factory(:viewer, :user => controller.current_user, :project => project)
    @request.env['HTTP_REFERER'] = 'http://referer/'
    post :force, :project_name => project.name
    response.should be_forbidden
    project.reload.should_not be_build_requested
  end
  ...
  [{:action => :show, :method => :get, :params => {:project_name => 'unknown_project'}}, {:action => :force, :method => :post, :params => {:project_name => 'unknown_project'}}].each do |entry|
    it "gives a 404 when an unknown project is requested for #{entry[:url]}" do
      project = Factory.create(:project, :name => 'existing_project')
